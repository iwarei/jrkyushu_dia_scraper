...
export const getLineCode = async (): Promise<LineInfo[]> => {
  try {
    const {data} = await axios.get('https://www.jrkyushu-timetable.jp/jr-k_time/top.html');
    ...
    $('tr td a').each((_, e) => {
      ...
      if (lineName.includes('線') && lineName.length === lineName.indexOf('線') + 1) {
 ...
 */
export const getStationInfo = async (lines: LineInfo[]): Promise<StationInfo[]> => {
